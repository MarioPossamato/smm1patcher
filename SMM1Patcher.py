...
def ChangeWingedSolidBlockSize(x, y, model=True, velocityHorizontal=True, velocityVertical=True, hitbox=True):
    with open(filename,'rb') as file:
        data = bytearray(filename.read())

        # Model Size
        if model == True:
            data[x['StartOffset']:x['StartOffset']+0x4] = bytes.fromhex(FloatToHex(y))
        elif model == False:
            pass
        else:
            data[x['StartOffset']:x['StartOffset']+0x4] = bytes.fromhex(FloatToHex(model))

        # Horizontal Velocity
        if velocityHorizontal == True:
            data[x['StartOffset']+0xC:x['StartOffset']+0x10] = bytes.fromhex(FloatToHex(48.0*y))
        elif velocityHorizontal == False:
            pass
        else:
            data[x['StartOffset']+0xC:x['StartOffset']+0x10] = bytes.fromhex(FloatToHex(48.0*velocityHorizontal))

        # Vertical Velocity
        if velocityVertical == True:
            data[x['StartOffset']+0x10:x['StartOffset']+0x14] = bytes.fromhex(FloatToHex(16.0*y))
        elif velocityVertical == False:
            pass
        else:
            data[x['StartOffset']+0x10:x['StartOffset']+0x14] = bytes.fromhex(FloatToHex(16.0*velocityVertical))

        # Hitbox Width Right, Height
        # Hitbox Width Left
        if hitbox == True:
            data[x['StartOffset']+0x18:x['StartOffset']+0x1C] = bytes.fromhex(FloatToHex(8.0*y))
            data[x['StartOffset']+0x24:x['StartOffset']+0x28] = bytes.fromhex(FloatToHex(-8.0*y))
        elif hitbox == False:
            pass
        else:
            data[x['StartOffset']+0x18:x['StartOffset']+0x1C] = bytes.fromhex(FloatToHex(8.0*hitbox))
            data[x['StartOffset']+0x24:x['StartOffset']+0x28] = bytes.fromhex(FloatToHex(-8.0*hitbox))
...
# Function To Change 1UP Mushrooms Into Extra Marios
...
x = {
"Key":0x000FAA00,
"HatenaBlock":0x000FAA08, # Crashes
"RengaBlock":0x000FAA18,
"Jumpstep":0x000FAA24,
"Jumpstep2":0x000FAA28,
"Pswitch":0x000FAA2C,
"Block_pow":0x000FAA30,
"Skewer":0x000FAA34,
"Coin":0x000FAA38, # Acts Like It Got Hit By A Pow Block Wave Or By A Bombhei Explosion
"Met":0x000FAA3C,
"Dossun":0x000FAA44,
"KillerHoudai":0x000FAA4C,
"SenkanHoudai":0x000FAA50,
"TogezoA":0x000FAA58,
"TogezoB":0x000FAA60,
"Poo":0x000FAA68,
"BlackPakkun":0x000FAA70,
"Wanwan":0x000FAA78,
"KoopaClown":0x000FAA7C,
"Yoshi":0x000FAA80,
"Mario":0x000FAA84, # Max Amount Of Extra Marios Is 2, For A Total Of Three Marios
"KitsuKuriboA":0x000FAA8C,
"KitsuKuriboA, Yoshi":0x000FAA90, # Depending On The Game Style
"KitsuKuriboB, Yoshi":0x000FAA94, # Depending On The Game Style
"Kuribo, Kuribon":0x000FAA9C, # Depending On The Game Style
"NokonokoA":0x000FAAAC,
"NokonokoB":0x000FAABC,
"Pakkun":0x000FAACC,
"PPakkun":0x000FAAD4,
"PakkunFire":0x000FAADC,
"Bros1st":0x000FAAEC,
"MegaBros":0x000FAAF0,
"Choropoo":0x000FAAF4,
"KoopaClownFireFireballProjectile":0x000FABD8,
"SpikeProjectile":0x000FABE4,
"BoneProjectile":0x000FABEC,
"WrenchProjectile":0x000FABF4,
"FireballProjectile":0x000FABF8,
"KoopaMWFallingFireballs":0x000FABFC
}

def Cherry(x=x['Mario']): # Found By Comex :)
    with open(filename,'rb') as file:
        data = bytearray(file.read())
        len(data)
        data[0x7FDCD0:0x7FDCD4] = int.to_bytes(0x38600001, 0x4, 'big')
        data[0x7FDC8C:0x7FDC90] = int.to_bytes(0x38600001, 0x4, 'big')
        data[0x265878:0x26587C] = int.to_bytes(0x60000000, 0x4, 'big')
        data[0x872100:0x872104] = int.to_bytes(0x60000000, 0x4, 'big')
        data[0x13561FC:0x1356200] = int.to_bytes(x, 0x4, 'big')
        with open(filename,'wb') as file:
            file.write(data)
        print('Ok')
...
def Read(x, y):
    with open(filename,'rb') as file:
        filename.seek(x['StartOffset'])
        data = filename.read(y)
...
"""
>>> Find(0x3f800000, start=0x1528000)
Ok
8101
9852
17485
...
def Find(x, start=0x00):
    with open(filename, 'rb') as file:
        file.seek(start)
        a = file.read()
        matches = [m.start() for m in re.finditer(bytes.hex(int.to_bytes(x, 0x4, 'big')), bytes.hex(a))]
        print('Ok')
...
"""
>>> Replace(0x3f800000, 0x40400000, 0x4, 0x00, 10)
Ok
"""
...
def Replace(x, y, z, start, a):
	with open(filename, 'rb') as file:
		...
		for i in range(int(a)):
			c[c.find(int.to_bytes(x, 0x4, 'big')):c.find(int.to_bytes(x, 0x4, 'big'))+z] = int.to_bytes(y, 0x4, 'big')
		with open(filename, 'wb') as file:
			file.write(b+c)
		print('Ok')
...
"""
>>> filename="Block.rpx"
>>> with open(filename+'.bak', 'rb') as file:
        file.write(data)
"""
